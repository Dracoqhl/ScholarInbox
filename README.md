...
当前项目已经有第一版可测试 MVP：可以定时或手动抓取 arXiv 日期范围论文、用本地预筛和批量 AI 终筛匹配研究兴趣、写入本地 SQLite、查看匹配论文列表、修改阅读状态、收藏论文、记录个人评论，并保存基础设置。
...
- 在网页中查看论文列表、中文摘要、PDF 详情解析、阅读状态、收藏和个人评论。
...
4. 在列表里修改阅读状态、点击星标收藏，或直接写下收藏理由、方向无关原因等个人评论；评论会自动保存，页面顶部会显示全局保存状态。
...
- SQLite 入库和去重
- 论文列表、首页中文解析、PDF 详情解析、阅读状态、方向无关标注、收藏、个人评论和新论文批量清理
- 抓取页和设置页合并
- 每日定时抓取
