...
当前项目已经有第一版可测试 MVP：可以定时或手动抓取 arXiv 日期范围论文、用本地预筛和批量 AI 终筛匹配研究兴趣、写入本地 SQLite、查看匹配论文列表、归档或收藏论文、添加自定义知识库标签、记录个人评论，并保存基础设置。
...
- 在网页中查看论文列表、中文摘要、PDF 详情解析、归档状态、收藏、自定义标签和个人评论。
...
4. 在列表里把新论文标记为 `归档`、`收藏` 或 `方向无关`，也可以添加彩色自定义标签；评论会自动保存，页面顶部会显示全局保存状态。
5. 打开 `/archive` 整理已归档论文，可按自定义标签、AI 关键词和论文日期筛选。
6. 打开 `/favorites` 回顾收藏论文；收藏论文会自动进入归档，只是重要性更高。
7. 在 `/settings` 点击 `测试 API` 检查 `.env.local` 中的模型 API 配置是否可用；当前连通性测试使用 Responses API。

抓取时会先用本地规则排除明显无关论文，再把可能相关论文按批次交给 AI 判断。过滤结果会保存到数据库，`/papers` 默认只显示匹配当前兴趣方向的论文，并按匹配分数优先排序，不保存自然语言筛选原因。

定时抓取由启动脚本托管的 `scripts/daily-crawl-scheduler.mjs` 执行。它不监听端口，只读取 `/crawls` 中保存的每日抓取时间和本地数据库状态；到达服务器本地时间的配置分钟时，会通过本机 API 发起一次 scheduled 抓取。定时抓取会抓取上一个服务器本地日期的 arXiv 论文；例如服务器在北京时间 `2026-06-03 06:00` 触发时，会抓取 `2026-06-02`。日期范围的开始日期和结束日期可以相同，这表示抓取这一天 `00:00` 到 `23:59` 的论文。
...
- SQLite 入库和去重
- 论文列表、首页中文解析、PDF 详情解析、新论文/归档/方向无关状态、收藏、用户自定义标签、归档筛选、个人评论和新论文批量清理
- 抓取页和设置页合并
- 每日定时抓取
