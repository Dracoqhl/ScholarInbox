...
1. 打开 `/settings`，确认 arXiv 分类和兴趣筛选文本，并点击 `测试 API` 确认 AI 配置可用。
2. 打开 `/crawls`，确认日期范围，点击开始抓取。手动抓取默认选择最近 7 天，避免 arXiv 当天尚未发布新条目时返回空结果。
...
抓取时会先用本地规则排除明显无关论文，再把可能相关论文按批次交给 AI 判断。过滤结果会保存到数据库，`/papers` 默认只显示匹配当前兴趣方向的论文，不保存自然语言筛选原因。
